...
from .config_manager.utils import AI_AGENT_DIR
...
class ServiceContext:
    ...
    def init_live2d(self, live2d_model_name: str) -> None:
        ...
        try:
            from .config_manager.utils import AI_AGENT_DIR
            model_dict_path = str(AI_AGENT_DIR / "model_dict.json")
            self.live2d_model = Live2dModel(live2d_model_name, model_dict_path=model_dict_path)
    ...
    def init_asr(self, asr_config: ASRConfig) -> None:
        if not self.asr_engine or (self.character_config.asr_config != asr_config):
            ...
            self.asr_engine = ASRFactory.get_asr_system(  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def init_tts(self, tts_config: TTSConfig) -> None:
        if not self.tts_engine or (self.character_config.tts_config != tts_config):
            ...
            self.tts_engine = TTSFactory.get_tts_engine(  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def init_vad(self, vad_config: VADConfig) -> None:
        ...
        if not self.vad_engine or (self.character_config.vad_config != vad_config):
            ...
            self.vad_engine = VADFactory.get_vad_engine(  # pyright: ignore[reportAttributeAccessIssue]
        ...
        for prompt_name, prompt_file in self.system_config.tool_prompts.items():
            ...
            if prompt_name == "live2d_expression_prompt":
                if self.live2d_model:
                    ...
                    # If Live2D is not initialized, remove the placeholder
                    prompt_content = prompt_content.replace("[<insert_emomap_keys>]", "")
    ...
    async def handle_config_switch(
        ...
        try:
            ...
            if config_file_name == "conf.yaml":
                # Load base config
                config_path = AI_AGENT_DIR / "conf.yaml"
                new_character_config_data = read_yaml(str(config_path)).get(
                    "character_config"
                )
            else:
                # Load alternative config and merge with base config
                characters_dir = self.system_config.config_alts_dir
                # Resolve characters_dir relative to AI_AGENT_DIR
                if os.path.isabs(characters_dir):
                    resolved_characters_dir = characters_dir
                else:
                    resolved_characters_dir = str(AI_AGENT_DIR / characters_dir)
                
                file_path = os.path.normpath(
                    os.path.join(resolved_characters_dir, config_file_name)
                )
                if not file_path.startswith(resolved_characters_dir):
