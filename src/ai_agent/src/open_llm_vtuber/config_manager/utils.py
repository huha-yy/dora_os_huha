...
# Get the ai_agent directory (parent of src directory)
# This file is at: src/ai_agent/src/open_llm_vtuber/config_manager/utils.py
# So we go up 4 levels to get to src/ai_agent/
AI_AGENT_DIR = Path(__file__).parent.parent.parent.parent
...
def scan_config_alts_directory(config_alts_dir: str) -> list[dict]:
    ...
    Parameters:
    - config_alts_dir (str): The path to the config_alts directory (relative to AI_AGENT_DIR).
    ...
    default_config_path = AI_AGENT_DIR / "conf.yaml"
    default_config = read_yaml(str(default_config_path))
    config_files.append(
        {
            "filename": "conf.yaml",
    ...
    # Resolve config_alts_dir relative to AI_AGENT_DIR
    if os.path.isabs(config_alts_dir):
        resolved_config_alts_dir = config_alts_dir
    else:
        resolved_config_alts_dir = str(AI_AGENT_DIR / config_alts_dir)

    # Scan other configs
    for root, _, files in os.walk(resolved_config_alts_dir):
        for file in files:
...
def scan_bg_directory() -> list[str]:
    bg_files = []
    bg_dir = str(AI_AGENT_DIR / "backgrounds")
...
def resolve_model_path(path: str) -> str:
    """
    Resolve a model path relative to AI_AGENT_DIR if it's a relative path.
