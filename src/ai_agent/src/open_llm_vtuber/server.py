...
from .config_manager.utils import Config, AI_AGENT_DIR
...
class WebSocketServer:
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        ws_router, self.ws_handler = init_client_ws_route(
        ...
        # Vision routes for external triggers
        self.app.include_router(init_vision_routes(ws_handler=self.ws_handler))
        ...
        cache_dir = str(AI_AGENT_DIR / "cache")
        ...
        self.app.mount(
            "/live2d-models",
            CORSStaticFiles(directory=str(AI_AGENT_DIR / "live2d-models")),
            name="live2d-models",
        )
        self.app.mount(
            "/bg",
            CORSStaticFiles(directory=str(AI_AGENT_DIR / "backgrounds")),
            name="backgrounds",
        )
        self.app.mount(
            "/avatars",
            AvatarStaticFiles(directory=str(AI_AGENT_DIR / "avatars")),
            name="avatars",
        )
        ...
        self.app.mount(
            "/web-tool",
            CORSStaticFiles(directory=str(AI_AGENT_DIR / "web_tool"), html=True),
            name="web_tool",
        )
        ...
        self.app.mount(
            "/",
            CORSStaticFiles(directory=str(AI_AGENT_DIR / "frontend"), html=True),
            name="frontend",
        )
