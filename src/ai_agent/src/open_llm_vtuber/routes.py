...
def init_client_ws_route(
    ...
    Returns:
        Tuple[APIRouter, WebSocketHandler]: Configured router and the ws_handler instance.
    """
...
def init_vision_routes(ws_handler: WebSocketHandler) -> APIRouter:
    """
    Create and return API routes for vision-related external triggers.

    Args:
        ws_handler: The WebSocket handler to trigger events on.

    Returns:
        APIRouter: Configured router with vision endpoints.
    """
    router = APIRouter()

    @router.post("/vision/fall-candidate")
    async def fall_detection_trigger():
        """
        Endpoint called by the CV service when a fall is detected.
        """
        logger.info("🚨 Fall detection API triggered!")
        alert_text = (
            "检测到用户可能摔倒。请确认用户是否安全，并询问是否需要帮助或紧急援助。"
        )
        await ws_handler.handle_external_trigger(alert_text)
...
def init_webtool_routes(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def get_live2d_folder_info():
        ...
        live2d_dir = str(AI_AGENT_DIR / "live2d-models")
