...
class WebSocketHandler:
    ...
    async def _handle_init_config_request(
        ...
        await websocket.send_text(
            json.dumps(
                {
                    "type": "set-model-and-conf",
                    "model_info": context.live2d_model.model_info if context.live2d_model else None,
    ...
    async def handle_external_trigger(self, text: str, client_uid: str = None) -> None:
        """
        Handle an external trigger (like fall detection) to interrupt and inform the LLM.

        Args:
            text: The text prompt describing the external event.
            client_uid: Optional specific client to trigger. If None, triggers all active clients.
        """
        target_uids = (
            [client_uid] if client_uid else list(self.client_connections.keys())
        )
        logger.debug(f"External trigger targets: {target_uids}")
        for uid in target_uids:
            if uid not in self.client_connections or uid not in self.client_contexts:
                continue

            websocket = self.client_connections[uid]
            context = self.client_contexts[uid]

            # 1. Interrupt ongoing tasks
            await self._handle_interrupt(
                websocket, uid, {"type": "interrupt-signal", "text": "External Trigger"}
            )

            # 2. Wake up the agent if it was sleeping
            if not context.is_awake:
                logger.info(f"Waking up AI {uid} for external trigger")
                context.is_awake = True
            context.update_activity_time()

            # 3. Cancel existing if any (already handled by _handle_interrupt above, but being safe)
            if uid in self.current_conversation_tasks:
                old_task = self.current_conversation_tasks[uid]
                if old_task and not old_task.done():
                    old_task.cancel()

            # 4. Route the trigger through the normal conversation flow so the LLM sees it.
