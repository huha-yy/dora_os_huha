...
class VoiceRecognition(ASRInterface):
    def __init__(
        ...
        self.hotwords = ""
        if self.hotwords_file and os.path.isfile(self.hotwords_file):
            if self.model_type == "transducer":
                try:
                    with open(self.hotwords_file, "r", encoding="utf-8") as f:
                        # sherpa-onnx hotwords are separated by "/"
                        words = [line.strip() for line in f if line.strip()]
                        self.hotwords = " / ".join(words)
                        logger.info(f"Loaded {len(words)} hotwords from {self.hotwords_file}")
                except Exception as e:
                    logger.error(f"Failed to load hotwords file: {e}")
            else:
                logger.warning(
                    f"Hotwords are provided but model type '{self.model_type}' does not support contextual biasing. "
                    "Only 'transducer' models support this feature. Hotwords will be ignored."
                )
    ...
    def _create_recognizer(self):
        if self.model_type == "transducer":
            ...
            if not self.sense_voice or not os.path.isfile(self.sense_voice):
                if self.sense_voice and (self.sense_voice.startswith(
                ...
                ) or "models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17" in self.sense_voice):
                    logger.warning(
                        "SenseVoice model not found. Downloading the model..."
                    )

                    # Get the chatbot root directory
                    ...
                    # Update the path after download
                    self.sense_voice = str(ROOT_DIR / "models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/model.int8.onnx")
                    self.tokens = str(ROOT_DIR / "models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/tokens.txt")
                else:
                    logger.critical(
    ...
    def transcribe_np(self, audio: np.ndarray) -> str:
        if self.hotwords and self.model_type == "transducer":
            stream = self.recognizer.create_stream(hotwords=self.hotwords)
        else:
