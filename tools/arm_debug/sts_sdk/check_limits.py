...
import msvcrt
print(f"角度限位读数: {min_val} ~ {max_val}")
print("   [Y] 强制清除限位 (设为 0~4095)   [N] 退出")
k = msvcrt.getch().decode().lower()
if k == 'y':
    print("\n正在清除角度限位...")
    packetHandler.unLockEprom(SID)
    time.sleep(0.05)
    packetHandler.write2ByteTxRx(SID, STS_MIN_ANGLE_LIMIT_L, packetHandler.sts_toscs(0, 15))
    time.sleep(0.05)
    packetHandler.write2ByteTxRx(SID, STS_MAX_ANGLE_LIMIT_L, packetHandler.sts_toscs(4095, 15))
    time.sleep(0.05)
